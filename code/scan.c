    ...
    chrs    s;
{
    ...
    while (c != NUL) {
        ...
        if (cur_cs == 0 && (cur_chr == c || cur_chr == c - 'a' + 'A')) {
            ...
            incr(s);
            c = *s;
        } else if (cur_cmd != SPACER || p != backup_tokens) {
            back_input();
            if (p != backup_tokens)
                back_list(token_link(backup_tokens));
            return FALSE;
        }
    }
    flush_list(token_link(backup_tokens));
    return TRUE;
}
    ...
    bool    negative;
{
    ...
    case DEF_CODE:
        ...
        if (m == MATH_CODE_BASE) {
            ...
            scanned_result(eqtb[m + cur_val].i, INT_VAL);
        }
        break;
    ...
    if (!shortcut) {
        ...
        if (cur_cmd >= MIN_INTERNAL && cur_cmd <= MAX_INTERNAL) {
            if (mu) {
                ...
                if (cur_val_level >= GLUE_VAL) {
                    v = width(cur_val);
                    delete_glue_ref(cur_val); cur_val = v;}
...
help_char ()
{
    help2("The numeric code for a character must be between 0 and 255.",
    "I changed this one to zero.");
}
